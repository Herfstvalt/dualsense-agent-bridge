...
struct FakeControllerInput {
    ...
    mutating func touch(
        ...
        _ phase: TouchpadPhase,
        x: Double,
        y: Double
    ) -> ControllerInput {
        .touchpad(touchEvent(contact, phase, x: x, y: y))
    }

    mutating func touchEvent(
        _ contact: TouchpadContact,
        _ phase: TouchpadPhase,
        x: Double,
        y: Double
        ...
        clock += 0.01
        return ControllerTouchpadEvent(
            controller: controller,
            contact: contact,
            phase: phase,
            position: TouchpadPosition(x: x, y: y),
            timestamp: clock,
            source: .fake
        )
    }
...
struct ActionRouterTests {
    ...
    /// Keeps hold lifecycle tests independent of the starter map. The hardware
    /// mic button is never emitted by GameController, so it is a safe fake-only
    /// control while both triggers are reserved for mouse buttons.
    ...
    func editingAndAccessibilityFallbacks() {
        var input = FakeControllerInput()
        var router = ActionRouter(profile: Self.wisprTestProfile)
    ...
    func holdBeginsAndEnds() {
        ...
        #expect(router.handle(input.press(.micButton)) == [.beginHold(wispr)])
        #expect(router.handle(input.release(.micButton)) == [.endHold(wispr)])
    ...
    func duplicatePressIsIdempotent() {
        ...
        #expect(router.handle(input.press(.micButton)) == [.beginHold(wispr)])
        #expect(router.handle(input.press(.micButton)).isEmpty)
    ...
    func duplicateReleaseIsHarmless() {
        ...
        _ = router.handle(input.press(.micButton))
        #expect(router.handle(input.release(.micButton)) == [.endHold(wispr)])
        #expect(router.handle(input.release(.micButton)).isEmpty)
    }
    ...
    func outOfOrderSequence() {
        ...
        let actions = [
            router.handle(input.release(.micButton)),
            router.handle(input.press(.micButton)),
            router.handle(input.press(.micButton)),
            router.handle(input.release(.micButton)),
            router.handle(input.release(.micButton)),
        ].flatMap { $0 }
    ...
    func unmappedControlIsReported() {
        ...
        #expect(router.handle(input.press(.create)) == [.unmapped(.create)])
        #expect(router.handle(input.release(.create)).isEmpty)
    }
    ...
    func disconnectEndsTheLeavingControllerFirst() {
        ...
        _ = router.handle(second.press(.micButton))
        _ = router.handle(first.press(.micButton))
    ...
    func survivingControllerCanPressAgain() {
        ...
        #expect(router.handle(second.release(.micButton)).isEmpty)
        #expect(router.handle(second.press(.micButton)) == [.beginHold(wispr)])
        #expect(router.handle(second.release(.micButton)) == [.endHold(wispr)])
        #expect(router.heldControlCount == 0)
    }
    ...
    func twoControlsSharingOneShortcut() throws {
        ...
        #expect(router.handle(input.press(.micButton)) == [.beginHold(shared)])
        #expect(router.handle(input.press(.r2)) == [.beginHold(shared)])
        #expect(router.heldControlCount == 2)

        #expect(router.handle(input.release(.micButton)) == [.endHold(shared)])
