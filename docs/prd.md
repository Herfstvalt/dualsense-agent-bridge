...
- Use Cross→Enter, Circle→Escape, Square→Backspace, R3→Ctrl-S, and touchpad
  ...
  session kill is never bound to a normal button. Use D-pad Left→Ctrl-Z,
  D-pad Right→Ctrl-C, and Options→Ctrl-V for the user's confirmed app-specific
  resize/undo, copy-last, and paste actions; document that Ctrl-C/Ctrl-Z retain
  their normal interrupt/suspend meanings when a plain terminal has focus.
